...
import { Bars3Icon, XMarkIcon, BellIcon } from '@heroicons/react/24/outline'; // Import icons from heroicons library
import logo from '../../assets/Logo-Deadlines.svg'; // Import logo image

const NavBar = () => {
  const [isOpen, setIsOpen] = useState(false); // State to manage the toggle state of the mobile menu
  ...
  return (
    <nav className="bg-butterscotch"> {/* Navigation bar with background color */}
      <div className="max-w-7xl mx-auto p-2 sm:px-6 lg:px-8"> {/* Container to center the content */}
        <div className="flex items-center justify-between h-16"> {/* Flex container to align items horizontally */}
          <div className="flex items-center"> {/* Flex container for logo and desktop menu items */}
            <Link to="/"> {/* Link to homepage */}
              <img src={logo} alt="Logo" className="h-16 w-16 mr-2" /> {/* Logo image */}
            </Link>
            <div className="hidden md:block"> {/* Hide on mobile, show on desktop */}
              <div className="ml-10 flex items-baseline space-x-4"> {/* Flex container for navigation links */}
                <Link to="/deadlines" className="text-custom-black hover:text-gray-200 px-3 py-2 rounded-md text-sm font-medium">Deadlines</Link> {/* Navigation link */}
                <Link to="/customize-quotes" className="text-custom-black hover:text-gray-200 px-3 py-2 rounded-md text-sm font-medium">Personnaliser</Link> {/* Navigation link */}
                <Link to="/quotes" className="text-custom-black hover:text-gray-200 px-3 py-2 rounded-md text-sm font-medium">Citations</Link> {/* Navigation link */}
                <Link to="/favoris" className="text-custom-black hover:text-gray-200 px-3 py-2 rounded-md text-sm font-medium">Favoris</Link> {/* Navigation link */}
                <Link to="/about" className="text-custom-black hover:text-gray-200 px-3 py-2 rounded-md text-sm font-medium">À propos</Link> {/* Navigation link */}
              </div>
            </div>
          </div>
          <div className="flex items-center space-x-6"> {/* Flex container for icons on the right */}
            ...
            <div className="relative"> {/* Notification icon with badge */}
              <button className="text-custom-black hover:text-gray-200"> {/* Button for notifications */}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-6 w-6"> {/* SVG icon */}
          ...
          <div className="-mr-2 flex md:hidden"> {/* Menu toggle button for mobile */}
            <button onClick={toggleMenu} className="text-custom-black hover:text-gray-200 inline-flex items-center justify-center p-2 rounded-md focus:outline-none">
              {isOpen ? <XMarkIcon className="h-6 w-6" /> : <Bars3Icon className="h-6 w-6" />} {/* Toggle icon */}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className={`md:hidden ${isOpen ? 'block' : 'hidden'}`}> {/* Mobile menu */}
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3"> {/* Mobile menu items */}
          ...
          <Link to="/quotes" className="text-custom-black hover:text-gray-200 block px-3 py-2 rounded-md text-base font-medium">Citations</Link>
          <Link to="/favoris" className="text-custom-black hover:text-gray-200 block px-3 py-2 rounded-md text-base font-medium">Favoris</Link>
        </div>
      </div>
    </nav>
  );
